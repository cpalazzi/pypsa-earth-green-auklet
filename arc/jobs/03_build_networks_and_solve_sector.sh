#!/bin/bash
#SBATCH --job-name=pypsa-earth-solve-sector
...
if [[ $# -lt 2 ]]; then
  echo "Usage: sbatch ../arc/jobs/03_build_networks_and_solve_sector.sh <run-label> <configfile> [configfile ...]" >&2
  echo "Example: sbatch ../arc/jobs/03_build_networks_and_solve_sector.sh europe-year-140-co2-zero-h2-sector configs/scenarios/config.europe-year-140-co2-zero-h2-sector.yaml" >&2
  exit 2
fi
...
LOGFILE="logs/snakemake-${SCENARIO}-$(date +%Y%m%d-%H%M%S)-solve-sector.log"
...
"$SNAKEMAKE" \
  ...
  --stats "logs/snakemake-${SCENARIO}-solve-sector.stats.json" 2>&1 | tee -a "$LOGFILE"
